...
public class chagneCamera1 : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Player.GetComponent<PlayerController>().isOverc2 )
        {
        ...
        if (started)
        {
            ...
            if(Input.GetKeyDown(KeyCode.Space)) 
            {
                ...
                rotable1.GetComponent<Rigidbody2D>().constraints = RigidbodyConstraints2D.FreezeAll;
                rotable2.GetComponent<Rigidbody2D>().constraints = RigidbodyConstraints2D.FreezeAll;

            }
            if(ball1.GetComponent<win>().wont && ball2.GetComponent<win>().wont)
            {
                MainCamera.SetActive(true);
                Camera2.SetActive(false);
                Player.GetComponent<PlayerController>().enabled = true;
